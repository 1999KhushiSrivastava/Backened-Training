...
// router.get("/films/:filmId", function(req, res){ // endpoint(Buisness logic ) (router)  , functiuon()--->route handler 
//     const films = [ {
//         "id": 1,
//         "name": "The Shining"
//        }, {
...
//        let filmId = req.params.filmId;   // params data identify , data fetch-->conditions provide --> dynamically use 
...
router.get("/sol1", function (req, res) {
  ...
  let result = 0
  for( var i in arr1){
  result +=arr1[i] //result=result+arr[i]
}
  let n = arr1.pop() //return remove element
  let consum=n*(n+1)/2
  let missingNumber= consum-result
...
router.get("/sol2", function (req, res) {
    ...
    let firstNumber = arr2[0]
    let lastNumber = arr2.pop()
    consum = (result+1)*(firstNumber+lastNumber)/2
    
     missingNumber = consum-sum
